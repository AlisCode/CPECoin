...
impl Block {
	...
	pub fn new_from_prev(prev_index: usize, prev_hash: String, data: BlockData) ->
																				Self {
		Block {
			index: prev_index + 1,
			data,
...
pub fn get_transactions() -> BlockData {
	...
	match File::open("transactions.cpecoin") {
		Ok(file) => {
			...
			let transactions = data.lines().map(|line| Transaction::parse_from_data(line.into())).collect();
